...
def calculate_mat(
    self, fact_hotel_tags_df, md_hotels_df, dim_source_segment_df, current_date
) -> BaseJobRunner.DataFrame:
    F = self.F
    # Convert business_date to date type
    fact_hotel_tags_df = fact_hotel_tags_df.withColumn(
        "business_date", F.to_date("business_date", "dd-MM-yyyy")
    ).filter(F.col("business_date") == current_date)

    # Step 2: Join with dimension tables
    transformed_df = (
        fact_hotel_tags_df.join(md_hotels_df, "hotel_code", "left")
