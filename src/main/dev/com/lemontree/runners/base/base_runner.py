...
from com.lemontree.configs.common_imports import DataFrame, F, T, W
from com.lemontree.utils.utils_helper_methods import *  # noqa
...
class BaseJobRunner(ABC):
    ...
    def __init__(self, args: Dict[str, Any]):
        ...
        self.config = load_config_for_job(self.job_name)  # noqa

        if self.config.get("job_type") is None:
            raise ValueError("Missing required argument in properties.yaml: 'job_type'")

        # initialize spark_session and glue_context in parent class for spark jobs so
        # that they are available to the child classes
