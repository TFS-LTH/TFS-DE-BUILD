from com.lemontree.constants.constants import (
    PRICE_GROUP_TYPES,
    RESERVATION_STATUS,
...
def calculate_rob(
    ...
    # ----------------------------
    dated_rsrv_curr_month = (
        ranked_rsrv_curr_month.filter(
            (F.col("rnk") == 1)
            & (F.col("departure_dt") >= F.lit(current_date))
            & (F.col("departure_dt") > F.col("arrival_dt"))
        )
        .withColumn("arrival_dt_new", F.col("arrival_dt"))
        .withColumn("departure_dt_new", F.col("departure_dt"))
    )
    ...
    rns_for_rsrv_curr_month = (
        ...
        .withColumn(
            ...
            F.explode(
                F.sequence(
                    F.col("arrival_dt_new").cast("date"),
        ...
        .join(
            fact_reservation_df,
            dated_rsrv_curr_month["bkg_num"] == fact_reservation_df["source_rsrv_id"],
            "left",
        )
        .join(
            source_segment_df,
            fact_reservation_df["src_sgmnt_id"] == source_segment_df["src_sgmnt_id"],
            "left",
        )
        .withColumn(
            "room_rate",
    ...
    # ----------------------------
    rob = (
        rns_for_rsrv_curr_month.groupBy(
            "stay_date",
            "hotel_id",
            "hotel_code",
            "no_of_rooms",
    ...
    rob = rob.replace(RESERVATION_STATUS_MAPPING, subset=["reservation_status"]).select(
