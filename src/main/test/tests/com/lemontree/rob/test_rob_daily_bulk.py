...
class TestRobFromCurrentDtToFuture(BaseTest):
    ...
    def test_calculate_rob(self):
        fact_reservation_df = (
            self.spark_session.read.format("csv").option("header", "true").load(str(Path(self.fact_reservation)))
        )
        md_hotels_df = self.spark_session.read.format("csv").option("header", "true").load(str(Path(self.md_mapping)))
        protel_reservation_df = (
            self.spark_session.read.format("csv").option("header", "true").load(str(Path(self.protel_reservation)))
        )
        source_segment_df = (
            self.spark_session.read.format("csv").option("header", "true").load(str(Path(self.source_segment_df)))
        )
        start_date = date.today()
        # get rob using sample data
        result = calculate_rob(
            self,
            fact_reservation_df,
            md_hotels_df,
            protel_reservation_df,
        ...
        test_rob_sum = (
            result.filter((self.F.col("stay_date") == self.start_date) & (self.F.col("hotel_id") == 27))
            .agg(self.F.sum("rob").alias("total_rob"))
            .first()["total_rob"]
        )
